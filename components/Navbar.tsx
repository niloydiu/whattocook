...
import { ChefHat, Search, ArrowRight, ClipboardList, Plus, Menu, X } from "lucide-react";
      ...
      <div className="max-w-7xl mx-auto px-3 sm:px-4 lg:px-8">
        ...
        <div className="h-14 sm:h-16 md:h-20 flex items-center justify-between gap-2 sm:gap-4">
          ...
          <div className="flex items-center gap-2 sm:gap-3 group flex-shrink-0">
            ...
            >
              <div className="w-7 h-7 sm:w-8 sm:h-8 md:w-10 md:h-10 bg-red-600 rounded-lg md:rounded-xl flex items-center justify-center text-white shadow-lg shadow-red-600/20 group-hover:rotate-12 transition-transform duration-300">
            ...
            {/* Search Bar - Desktop */}
            <div className="relative items-center bg-white/80 backdrop-blur-sm border border-slate-200/60 rounded-2xl px-3 lg:px-4 py-2 lg:py-2.5 w-full max-w-[200px] lg:max-w-xs group focus-within:ring-2 focus-within:ring-red-500/30 focus-within:border-red-300/50 transition-all shadow-sm hover:shadow-md flex">
              <Search
                size={16}
              ...
              <AnimatePresence>
                {showSuggestions && suggestions.length > 0 && (
                  <motion.div
                    ...
                    exit={{ opacity: 0, y: 10 }}
                    className="absolute top-full left-0 right-0 mt-2 bg-white rounded-2xl shadow-2xl border border-slate-100/80 overflow-hidden z-[120] backdrop-blur-xl max-h-80 overflow-y-auto"
                  >
                    {suggestions.map((s, i) => (
                      <button
                        key={i}
                        onClick={() => onSelectSuggestion(s)}
                        className="w-full px-3 lg:px-4 py-2.5 lg:py-3 text-left hover:bg-red-50/50 active:bg-red-50 flex items-center gap-2 lg:gap-3 transition-all duration-200 border-b border-slate-50 last:border-0"
                      >
                        {s.type === "category" ? (
            ...
            <Link
              href="/request-recipe"
              className="hidden lg:flex items-center gap-2 px-4 lg:px-5 py-2 lg:py-2.5 bg-gradient-to-r from-red-600 to-orange-600 text-white rounded-xl lg:rounded-2xl font-bold text-xs lg:text-sm hover:from-red-700 hover:to-orange-700 active:scale-95 transition-all shadow-lg shadow-red-600/30 hover:shadow-xl hover:shadow-red-600/40 whitespace-nowrap"
            >
              <ClipboardList className="w-4 h-4 lg:w-5 lg:h-5" />
              <span>{locale === "en" ? "Request" : "অনুরোধ"}</span>
            </Link>

            {/* Language Toggle */}
            <div className="flex items-center bg-slate-100/60 p-0.5 lg:p-1 rounded-xl lg:rounded-2xl backdrop-blur-sm border border-slate-200/60 shadow-sm">
              <button
        ...
        <AnimatePresence>
          {mobileSearchOpen && (
            <motion.div
              ...
              className="md:hidden border-t border-slate-200/60 overflow-hidden"
            >
              <div className="px-3 py-3 relative">
                <div className="relative flex items-center bg-white/80 backdrop-blur-sm border border-slate-200/60 rounded-xl px-3 py-2.5 focus-within:ring-2 focus-within:ring-red-500/30 focus-within:border-red-300/50 transition-all shadow-sm">
                  <Search
                    size={18}
                ...
                <AnimatePresence>
                  {showSuggestions && suggestions.length > 0 && (
                    <motion.div
                      ...
                      animate={{ opacity: 1, y: 0 }}
                      exit={{ opacity: 0, y: -10 }}
                      className="absolute left-3 right-3 top-full mt-2 bg-white rounded-xl shadow-2xl border border-slate-100/80 overflow-hidden z-[120] backdrop-blur-xl max-h-64 overflow-y-auto"
                    >
                      {suggestions.map((s, i) => (
                        <button
                          key={i}
                          ...
                          className="w-full px-3 py-2.5 text-left hover:bg-red-50/50 active:bg-red-50 flex items-center gap-2 transition-all duration-200 border-b border-slate-50 last:border-0"
                        >
                          {s.type === "category" ? (
            ...
            >
              <div className="px-3 py-3 space-y-2">
                ...
                <Link
                  ...
                  className="flex items-center gap-3 px-4 py-3 bg-gradient-to-r from-red-600 to-orange-600 text-white rounded-xl font-bold text-sm hover:from-red-700 hover:to-orange-700 active:scale-95 transition-all shadow-lg shadow-red-600/30"
                >
                ...
                <div className="flex items-center gap-2 px-4 py-2 bg-slate-50 rounded-xl">
                  ...
                  <div className="flex items-center bg-white p-0.5 rounded-lg border border-slate-200/60 shadow-sm">
                    <button
                      onClick={() => setLocale("en")}
