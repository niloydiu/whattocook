...
export async function queryRecipesByPantry(pantry: string[], locale: 'en' | 'bn' = 'en', limit = 100): Promise<RecipeMatch[]> {
  if (!supabaseAdmin) throw new Error('supabaseAdmin is not configured; set SUPABASE_SERVICE_ROLE_KEY')
  ...
  if (normPantry.length === 0) {
    ...
    const { data } = await supabaseAdmin.from('recipes').select('*').limit(limit)
    return (data || []).map((r: any) => ({ recipe: r as RecipeRow, matches: 0, total: (r[col]||[]).length, score: 0, missing: r[col] || [] }))
  }
  ...
  results.sort((a, b) => (b.score === a.score ? a.missing.length - b.missing.length : b.score - a.score))
